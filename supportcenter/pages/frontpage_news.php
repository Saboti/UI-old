...
$main_html .= '<span class=header>Frontpage Newsmeldungen</span><br>';

if(isset($_REQUEST['remove'])) {
    $msg = $db->queryrow('SELECT * FROM page_news WHERE id="'.((int)$_REQUEST['id']).'"');
    log_action('Page-News mit dem Titel "'.$msg['header'].'" gel&ouml;scht');


    $sql = 'DELETE FROM page_news WHERE id="'.((int)$_REQUEST['id']).'" LIMIT 1';
    if(!$db->query($sql)) {
...
if(isset($_POST['submit'])) {
    ...
    {
         $sql = 'INSERT INTO page_news (header, news, date)
                 VALUES ("'.$_POST['title'].'", "'.addslashes($_POST['text']).'", '.$send_time.')';

         log_action('Page-News mit dem Titel "'.$_POST['title'].'" geschrieben');
...
<form method="post" action="index.php?p=frontpage_news">
...
';
    ...
    if(($q_news = $db->query($sql)) === false) {
        ...
        message(DATABASE_ERROR, 'Could not query page news data');
    ...
    while($news = $db->fetchrow($q_news)) {
      ...
      <table border="0" cellpadding="0" cellspacing="0" width="100%" class="style_inner">
        ...
        <tr>
		  ...
		  <br><br>
		  [<a href="index.php?p=frontpage_news&id='.$news['id'].'"><font color=white>&Auml;ndern</font></a>]<br><br>
		  [<a href="index.php?p=frontpage_news&id='.$news['id'].'&remove"><font color=white>L&ouml;schen</font></a>]<br>
